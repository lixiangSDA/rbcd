...
class SearchRBCD(object):
        ...
        for i in range(0,searchlength):
            ...
            if searchresult['msDS-AllowedToActOnBehalfOfOtherIdentity']:
                ...
                for ace in sd['Dacl'].aces:
                    ...
                    print(AllowedToActOnBehalfOfOtherIdentitySID)
    ...
    def get_computer_CreatorSID(self):
        root = self.ldap_server.info.other['defaultNamingContext'][0]
        #self.ldap_session.search(root,"(&(objectCategory=computer)(objectClass=computer))",attributes=['cn','msDS-AllowedToActOnBehalfOfOtherIdentity'])
        self.ldap_session.search(root,"(&(objectCategory=computer)(objectClass=computer))",attributes=['cn','mS-DS-CreatorSID'])
        searchlength = len(self.ldap_session.entries)
        ldap_entriess.append(self.ldap_session.entries)
        first_ldap_entriess = ldap_entriess[0]
        for i in range(0,searchlength):
            searchresult = first_ldap_entriess[i]
            computer = searchresult['cn']
            if searchresult['mS-DS-CreatorSID']:
                mS_DS_CreatorSID = searchresult['mS-DS-CreatorSID'][0]
                sid = win32security.SID(mS_DS_CreatorSID)
                mSDSCreatorSID = win32security.ConvertSidToStringSid(sid)
                SamAccountNameList = self.get_sid_info(root,mSDSCreatorSID)
                if (SamAccountNameList):
                     print(f'computer:{computer} \'s mS-DS-CreatorSID value is {mSDSCreatorSID} , sidname is {SamAccountNameList[1]}')
...
def main():
    ...
    parser.add_argument('-m','--mode', help='default is msDS-AllowedToActOnBehalfOfOtherIdentity,csid is mS-DS-CreatorSID', required=False)
    ...
    try:
        ...
        if (args.mode == 'csid'):
            searchrbcd.get_computer_CreatorSID()
        else:
